...
export default function Breadcrumbs() {
  ...
  return <div className='text-xs flex flex-row justify-start items-center gap-1 text-gray-600 pb-2'>
    <LayoutDashboard size={14} /> {
      paths.map((path, index) => {
        ...
        return (<div key={index} className='flex flex-row justify-start items-center gap-1'>
          <div className={`${index === paths.length-1 && 'text-mila_orange font-bold'}`}>{pageName}</div>
