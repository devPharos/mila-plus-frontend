...
export default function Filters({ handleNew = null, search, handleFilters = null, gridHeader = null, setGridHeader = () => null, gridData = null, activeFilters = null }) {
        ...
        // if(type === 'text') {
            return (
              ...
              {options.map((option, elIndex) => {
                return <button type='button' key={elIndex} onClick={() => {setActivePopover('');handleFilters({title ,value: option})}} className={`${active.length > 0 && active[0].value === option && 'bg-gray-300'} hover:bg-gray-300 px-2 py-2 rounded text-left whitespace-nowrap`}>{option}</button>
              })}
    ...
    return <div className='flex flex-row justify-between items-center w-full'>
        <button type='button' onClick={() => handleNew()} className='p-2 border border-mila_orange text-mila_orange font-bold rounded text-xs'>+ New Filial</button>
