...
const Input = ({ name, readOnly, grow = false, ...rest }) => {
  ...
  return (
    <div className="flex flex-col justify-center items-center relative">
      <div
        ...
        className={`${
          grow ? "w-full" : "w-72"
        } border rounded-full p-2 px-4 text-sm flex flex-row justify-between items-center gap-2 bg-white ${
          error && "border-red-300"
        }`}
      >
        {name === "email" && <AtSign size={12} color="#9ca3af" />}
        {name === "password" && <Key size={12} color="#9ca3af" />}
        {name === "confirmPassword" && <Key size={12} color="#9ca3af" />}
        <input
          id={name}
          name={name}
          ref={inputRef}
