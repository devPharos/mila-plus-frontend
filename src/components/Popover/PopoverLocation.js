...
export default function PopoverLocation() {
  ...
  return <><div className='absolute top-9 left-2 bg-secondary rounded-xl w-36 shadow-xl p-2 text-xs *:border-b last:border-b-0'>
    {profile.filials.map(({ filial: filArray }, index) => {
      return <button key={index} type="button" onClick={() => { handleFilialChange(filArray); setActivePopover('') }} className={`w-full flex flex-row justify-start items-center border gap-2 hover:bg-slate-100 px-2 py-1 rounded ${filial.alias === filArray.alias && 'bg-white'}`}>
