...
export default function PagePreview({ access, id, handleOpened, setOpened, defaultFormType = 'preview' }) {
    ...
    const typeOptions = [{ value: 'Administrative', label: 'Administrative' }, { value: 'Academic', label: 'Academic' }]
    ...
    async function handleGeneralFormSubmit(data) {
        ...
        if (id === 'new') {
            try {
                ...
                const response = await api.post(`/calendar-days`, { ...data, day: day ? format(day, 'yyyy-MM-dd') : null, dayto: dayto ? format(dayto, 'yyyy-MM-dd') : null })
            ...
            if (updated.length > 0) {
                ...
                try {
                    const { day, dayto } = data;
                    await api.put(`/calendar-days/${id}`, { ...objUpdated, day: day ? format(day, 'yyyy-MM-dd') : null, dayto: dayto ? format(dayto, 'yyyy-MM-dd') : null })
        ...
        {pageData ?
            formType === 'preview' ? <div className='border h-full rounded-xl overflow-hidden flex flex-col justify-start gap-1 overflow-y-scroll'>
                ...
                <div className='flex flex-1 flex-col items-left px-4 py-2 gap-1'>
                    ...
                    <div className='flex flex-row items-center gap-1 text-xs'><strong>{pageData.dayto ? 'From:' : 'Date:'}</strong> {format(parseISO(pageData.day), 'MMMM, do')}</div>
                    {pageData.dayto ? <div className='flex flex-row items-center gap-1 text-xs'><strong>To:</strong> {format(parseISO(pageData.dayto), 'MMMM, do')}</div> : null}
                    <p className='border-b mt-2 mb-1 pb-1'>Content</p>
                    <div className='flex flex-row items-center gap-1 text-xs'><strong>Type:</strong> {pageData.type}</div>
                        ...
                        <div className='flex flex-col items-start justify-start text-sm overflow-y-scroll'>
                            <Form ref={generalForm} onSubmit={handleGeneralFormSubmit} className='w-full'>
                                <InputContext.Provider value={{ id, generalForm, setSuccessfullyUpdated, fullscreen, setFullscreen, successfullyUpdated, handleCloseForm, handleInactivate, canceled: pageData.canceled_at }}>
                                    {pageData.loaded ?
                                        <>
                                            <FormHeader access={access} title={pageData.title} registry={registry} InputContext={InputContext} />
                                            <InputLineGroup title='GENERAL' activeMenu={activeMenu === 'general'}>
                                                {auth.filial.id === 1 && <InputLine title='Filial'>
                                                    <SelectPopover name='filial_id' required title='Filial' isSearchable defaultValue={filialOptions.find(filial => filial.value === pageData.filial_id)} options={filialOptions} InputContext={InputContext} />
                                                </InputLine>}
                                                <InputLine title='General Data'>
                                                    <DatePicker name='day' title='Date' required defaultValue={pageData.day ? parseISO(pageData.day) : null} placeholderText='MM/DD/YYYY' InputContext={InputContext} />
                                                    <DatePicker name='dayto' title='Date' defaultValue={pageData.dayto ? parseISO(pageData.dayto) : null} placeholderText='MM/DD/YYYY' InputContext={InputContext} />
                                                    <SelectPopover name='type' required title='Type' defaultValue={typeOptions.find(type => type.value === pageData.type)} options={typeOptions} InputContext={InputContext} />
                                                    <Input type='text' name='title' required grow title='Title' defaultValue={pageData.title} InputContext={InputContext} />
                                                </InputLine>
