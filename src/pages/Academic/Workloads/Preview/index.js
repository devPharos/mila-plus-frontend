...
export default function PagePreview({ access, id, handleOpened, setOpened, defaultFormType = 'preview' }) {
    ...
    useEffect(() => {
        async function getPageData() {
            try {
                ...
                const totalClasses = Math.round(data.Level.total_hours / data.hours_per_day) || 0;
                const defaultPaceGuides = [];
                for (let classDay = 1; classDay < totalClasses + 1; classDay++) {
                    defaultPaceGuides.push({ day: classDay, data: [] })
                }
                const { data: paceGuidesData } = await api.get(`paceguides_by_workload/${id}`)
        ...
        async function getLevelsOptions() {
            try {
                ...
                const levels = data.map(({ id, name, total_hours, Programcategory }) => {
                    const programcategory_name = Programcategory.name;
                    return { value: id, label: programcategory_name + ' - ' + name + ' (' + total_hours.toString() + 'h)' }
                })
    ...
    function handleRemoveContent(day, index) {
        ...
        newData.paceGuides.filter(pace => pace.day == day)[0].data.splice(index, 1);
                    ...
                    formType === 'preview' ? <div className='border h-full rounded-xl overflow-hidden flex flex-col justify-start gap-1 overflow-y-scroll'>
                        ...
                        <div className='flex h-full flex-row items-start justify-between gap-4'>
                            ...
                            <div className='flex flex-col items-center justify-between text-xs w-32 gap-4'>
                                ...
                                <RegisterFormMenu setActiveMenu={setActiveMenu} disabled={id === 'new'} messageOnDisabled='Create the workload to have access to Pace Guide.' activeMenu={activeMenu} name='Pace Guide' >
