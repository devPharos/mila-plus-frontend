...
export default function FilialTypesPreview({ id, handleOpened, setOpened, defaultFormType = 'preview' }) {
    ...
    async function handleGeneralFormSubmit(data) {
        ...
        if (id === 'new') {
            try {
                const response = await api.post(`/filialtypes`, data)
            ...
            if (updated.length > 0) {
                ...
                try {
                    await api.put(`/filialtypes/${id}`, objUpdated)
    ...
    useEffect(() => {
        async function getPageData() {
            try {
                const { data } = await api.get(`filialtypes/${id}`)
                setPageData(data)
