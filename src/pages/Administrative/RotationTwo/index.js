...
export default function RotationTwo() {
  ...
  async function handleFinishRotation() {
    for (let group of newGroups) {
      if (group.classroom_id === null || group.teacher_id === null) {
        alertBox({
          title: "Attention!",
          descriptionHTML:
            "Please define a classroom and a teacher for each class.",
          buttons: [
            {
              title: "Ok",
              class: "cancel",
                              ...
                              {group?.rotations?.filter(
                                (rotation) =>
                                  rotation.student.medical_excuses.length ===
                                    0 && rotation.student.vacations.length === 0
                              )?.length || 0}
                            </div>
                          </div>
                        );
                      ...
                      >
                        ...
                        <div className="text-left w-full px-2 border-b border-zinc-100 pb-2">
                          ...
                          >
                            ...
                            {filterOptions?.teachers?.map((teacher, index) => {
                              return (
                                <option
                                  value={teacher.value}
                                  key={index}
