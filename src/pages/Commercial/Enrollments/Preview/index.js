    ...
    async function handleGeneralFormSubmit(data) {
        ...
        if (id === 'new') {
            try {
                ...
                const response = await api.post(`/enrollments`, { ...data, date_of_birth: date_of_birth ? format(date_of_birth, 'yyyy-MM-dd') : null, visa_expiration: visa_expiration ? format(visa_expiration, 'yyyy-MM-dd') : null })
            ...
            if (updated.length > 0) {
                ...
                try {
                    await api.put(`/enrollments/${id}`, { ...objUpdated, date_of_birth: date_of_birth ? format(date_of_birth, 'yyyy-MM-dd') : null, visa_expiration: visa_expiration ? format(visa_expiration, 'yyyy-MM-dd') : null })
                                                ...
                                                {pageData.enrollmenttimelines && pageData.enrollmenttimelines.length > 0 && pageData.enrollmenttimelines.map((timeline, index) => {
                                                    return <Scope key={index} path={`enrollmenttimelines[${index}]`}>
                                                        <InputLine title={format(timeline.created_at, 'MM/dd/yyyy @ HH:mm')}>
                                                            ...
                                                            <DatePicker name='expected_date' grow title='Expected Date' defaultValue={timeline.expected_date ? parseISO(timeline.expected_date) : null} placeholderText='MM/DD/YYYY' InputContext={InputContext} />
                                                        </InputLine>
                                                    </Scope>
                                                })}
                                            </InputLineGroup>
