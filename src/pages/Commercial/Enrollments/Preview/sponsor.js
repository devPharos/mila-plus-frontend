...
export default function SponsorOutside({ access = null, handleOpened, setOpened, defaultFormType = 'preview' }) {
    ...
    const menus = [{ order: 8, name: 'sponsor-signature' }]
    ...
    useEffect(() => {
        async function getDocuments(type = '') {
            const { data } = await api.get(`/documentsByOrigin?origin=Enrollment&type=${type}&subtype=Sponsor`)
            return data;
        }
            ...
            await uploadString(imageRef, signature.substring(22), 'base64')
                .then(async (snapshot) => {
                    await getDownloadURL(snapshot.ref).then(async (downloadURL) => {
                        ...
                        await api.put(`/outside/sponsors/${id}`, { activeMenu: pageData.activeMenu, lastActiveMenu: pageData.lastActiveMenu })
                ...
                Promise.all(allPromises).then(async (files) => {
                    try {
                        files.map(async (file) => {
                            ...
                            if (file.name) {
                                api.post(`/enrollmentdocuments`, { enrollment_id: pageData.id, files: file })
                                ...
                                file.sort((a, b) => a.size > b.size).map(async (promise, index) => {
                                    await Promise.all([promise]).then(async (singleFile) => {
                                        ...
                                        await api.post(`/enrollmentdocuments`, { enrollment_id: pageData.id, files: singleFile[0] })
                                    })
                                })
                            }
                        })
                    ...
                    await api.put(`/outside/sponsors/${id}`, { ...data, activeMenu: pageData.activeMenu, lastActiveMenu: pageData.lastActiveMenu, birthday: birthday ? format(birthday, 'yyyyMMdd') : null })
        ...
        {!sent && pageData.loaded ?
            <div className='flex h-full flex-col items-start justify-between gap-4 md:flex-row'>
                ...
                <div className='flex flex-row items-center justify-between text-xs w-32 gap-4 md:flex-col'>
                    <RegisterFormMenu disabled={false} activeMenu={pageData.activeMenu} name='sponsor-signature' >
                ...
                <div className='border h-full rounded-xl overflow-hidden flex flex-1 flex-col justify-start'>
                    <div className='flex flex-col items-start justify-start text-sm overflow-y-scroll h-full'>
                        <Form ref={generalForm} onSubmit={handleGeneralFormSubmit} className='w-full h-full'>
                            <InputContext.Provider value={{ id, generalForm, setSuccessfullyUpdated: () => null, fullscreen, setFullscreen, successfullyUpdated, handleCloseForm, handleInactivate, handleOutsideMail, canceled: pageData.canceled_at }}>
                                {pageData.loaded ?
                                    <>
                                        ...
                                        {pageData.activeMenu === 'sponsor-signature' && !pageData.enrollmentsponsors[0].signature ? <InputLineGroup title='Sponsor Signature' activeMenu={pageData.activeMenu === 'sponsor-signature'}>
                                            <InputLine title='Sponsor Information'>
                                                <Input type='text' name='name' required grow title='Full Name' defaultValue={pageData.enrollmentsponsors[0].name} InputContext={InputContext} />
                                                <Input type='text' name='email' required grow title='E-mail' defaultValue={pageData.enrollmentsponsors[0].email} InputContext={InputContext} />
                                                <Input type='text' name='phone' required grow title='Phone Number' isPhoneNumber defaultValue={pageData.enrollmentsponsors[0].phone} InputContext={InputContext} />
                                                <DatePicker name='birthday' required grow title='Birth Date' defaultValue={pageData.enrollmentsponsors[0].birthday ? parseISO(pageData.enrollmentsponsors[0].birthday) : null} placeholderText='MM/DD/YYYY' InputContext={InputContext} />
                                            </InputLine>
                                            <InputLine title='Address Information'>
                                                <Input type='text' name='address' required grow title='Address' defaultValue={pageData.enrollmentsponsors[0].address} InputContext={InputContext} />
                                                <Input type='text' name='zip_code' required grow title='Zip Code' defaultValue={pageData.enrollmentsponsors[0].zip_code} InputContext={InputContext} />
                                            </InputLine>
                                            <InputLine>
                                                <Input type='text' name='city' required grow title='City' defaultValue={pageData.enrollmentsponsors[0].city} InputContext={InputContext} />
                                                <Input type='text' name='state' required grow title='State' defaultValue={pageData.enrollmentsponsors[0].state} InputContext={InputContext} />
                                                <SelectPopover name='country' required grow title='Country' isSearchable defaultValue={countriesOptions.find(country => country.value === pageData.enrollmentsponsors[0].country)} options={countriesOptions} InputContext={InputContext} />
                                            </InputLine>
                                            <InputLine title='Birth Place Information'>
                                                <Input type='text' name='birth_city' required grow title='City' defaultValue={pageData.enrollmentsponsors[0].birth_city} InputContext={InputContext} />
                                                <Input type='text' name='birth_state' required grow title='State' defaultValue={pageData.enrollmentsponsors[0].birth_state} InputContext={InputContext} />
                                                <SelectPopover name='birth_country' required grow title='Country' isSearchable defaultValue={countriesOptions.find(country => country.value === pageData.enrollmentsponsors[0].birth_country)} options={countriesOptions} InputContext={InputContext} />
                                            </InputLine>
                                            <InputLine title='Proof of financial support'>
                                                <div className='text-md'><strong>Name of the student:</strong> {pageData.students.name}</div>
                                            </InputLine>
                                            <InputLine>
                                                <div className='text-md'><strong>Country of citizen:</strong> {pageData.students.citizen_country}</div>
                                            </InputLine>
                                            <InputLine>
                                                <div>Immigration requires that students submit documented proof of financial support. <em>MILA School</em> requires the amount indicated below:</div>
                                            </InputLine>
                                            <InputLine>
                                            ...
                                            <InputLine>
                                                {pageData.enrollmentsponsors[0].responsible_checkbox ?
                                                    <CheckboxInput grow name='responsible_checkbox' defaultValue={true} required title={`I will be responsible for student's financial and personal support for the durantion of hir/her studies at MILA.`} InputContext={InputContext} />
                                                    :
                                                    <CheckboxInput grow name='responsible_checkbox' required title={`I will be responsible for student's financial and personal support for the durantion of hir/her studies at MILA.`} InputContext={InputContext} />
                                                }
                                            </InputLine>
