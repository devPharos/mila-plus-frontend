...
export default function PagePreview({
  ...
  async function handleGeneralFormSubmit(data) {
    ...
    if (id === "new") {
      try {
        const response = await api.post(`/merchantxchartofaccount`, data);
      ...
      if (updated.length > 0) {
        ...
        try {
          await api.put(`/merchantxchartofaccount/${id}`, objUpdated);
  ...
  useEffect(() => {
    async function getPageData() {
      try {
        const { data } = await api.get(`/merchantxchartofaccount/${id}`);
    ...
    async function getDefaultOptions() {
      try {
        ...
        const merchantOptions = merchantData.data
          .filter((m) => m.id !== id)
          .map((m) => {
