...
export default function FinancialMerchantsChartOfAccounts() {
  ...
  useEffect(() => {
    async function getBankAccounts() {
      const { data } = await api.get("/merchantxchartofaccount");
      const gridDataValues = data.map(
        ({ id, merchant_id, merchant, chartofaccount_id, chartOfAccount, filial_id, filial }) => {
          return {
            show: true,
            id,
            fields: [ merchant.name, chartOfAccount.name, filial.name ],
          }
        }
      );
