import { ChevronDown, ChevronUp } from "lucide-react";
import { useState } from "react";
import DatePicker from "react-datepicker";
import useReportsStore from "~/store/reportsStore";

export default function PeriodFilter() {
  const [open, setOpen] = useState(false);
  const { filters, setFilters, periodOptions } = useReportsStore();
  const [startDate, setStartDate] = useState(filters.period.from_date);
  const [endDate, setEndDate] = useState(filters.period.until_date);
  const [selected, setSelected] = useState(filters.period);
  const onChange = (dates) => {
    const [start, end] = dates;
    setStartDate(start);
    setEndDate(end);
  };
  return (
    <div className="flex flex-row justify-start items-start gap-2 h-16">
      {open && (
        <div
          onClick={() => setOpen(false)}
          className="fixed top-0 left-0 w-full h-full bg-transparent z-1"
        ></div>
      )}
      <div
        className={`flex flex-col gap-y-2 text-xs bg-zinc-100 rounded p-1 min-h-12 min-w-48 z-10 border ${
          open ? "border-primary shadow-lg" : "border-transparent"
        }`}
      >
        <button
          type="button"
          onClick={() => setOpen(!open)}
          className="flex flex-row justify-between items-center gap-x-2 text-xs cursor-pointer p-1 text-left"
        >
          <div className="flex flex-col">
            <span>Period</span>
            <strong className="text-sm">{selected?.label}</strong>
            {/* {selected.label !== "Custom" && (
              <>
                <br />
                {selected.from
                  ? format(selected.from, "MM/dd/yyyy")
                  : null} -{" "}
                {selected.to ? format(selected.to, "MM/dd/yyyy") : null}
              </>
            )} */}
          </div>
          {open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        </button>
        {open && (
          <div className="flex flex-col justify-start items-start gap-2 py-1 border-t border-zinc-300 text-zinc-500">
            {periodOptions.map((option, index) => (
              <button
                key={index}
                type="button"
                onClick={() => {
                  setFilters({
                    ...filters,
                    period: option,
                  });
                  setOpen(false);
                  setSelected(option);
                }}
                className="w-full text-left p-1 hover:text-primary hover:bg-zinc-200 rounded cursor-pointer"
              >
                {option?.label}
              </button>
            ))}
          </div>
        )}
      </div>
      {selected?.label === "Custom" && (
        <div className="flex flex-row justify-between items-center gap-x-2 text-xs cursor-pointer text-left p-1">
          <DatePicker
            className="bg-transparent bg-zinc-50 border text-center border-zinc-200 rounded h-12"
            selected={startDate}
            onChange={onChange}
            startDate={startDate}
            endDate={endDate}
            selectsRange
            rangeSeparator=" to "
          />
        </div>
      )}
    </div>
  );
}
